...
# An automation referencing the same device at least this many times is flagged
# as tightly coupled to it.
_TIGHT_COUPLING_MIN_REFS = 3
# A called script with more than this many onward dependencies hides a long chain.
_HIDDEN_DEPENDENCY_MIN_DEPS = 2
...
def _detect_device_id_usage(graph: DependencyGraph) -> list[FragilityFinding]:
    ...
    for edge in graph.edges:
        if edge.dependency_kind not in device_ref_kinds:
            continue

        source_node = graph.nodes.get(edge.source)

        if not source_node or source_node.node_type not in (NodeType.AUTOMATION, NodeType.SCRIPT):
            continue
...
def _detect_tight_device_coupling(
    ...
    for edge in graph.edges:
        ...
        if not source_node or source_node.node_type != NodeType.AUTOMATION:
            continue

        auto_device_refs.setdefault(edge.source, Counter())[edge.target] += 1

    for auto_id, device_counts in auto_device_refs.items():
        findings.extend(_tight_coupling_findings(auto_id, device_counts))

    return findings


def _tight_coupling_findings(auto_id: str, device_counts: Counter[str]) -> list[FragilityFinding]:
    """Flag each device an automation references at least three times."""
    findings: list[FragilityFinding] = []

    for device_id, count in device_counts.items():
        if count < _TIGHT_COUPLING_MIN_REFS:
            continue
        ...
        findings.append(
            FragilityFinding(
                ...
                remediation=(
                    "Refactor this automation to use entity-based references where possible."
                ),
            )
        )
...
def _detect_hidden_dependencies(
    ...
    for edge in graph.edges:
        ...
        if source_node.node_type != NodeType.AUTOMATION or target_node.node_type != NodeType.SCRIPT:
            continue

        script_deps = graph.get_outbound(edge.target)

        if len(script_deps) <= _HIDDEN_DEPENDENCY_MIN_DEPS:
            continue
        ...
        findings.append(
            FragilityFinding(
                ...
                remediation=("Review the called script to understand the full dependency chain."),
            )
        )
